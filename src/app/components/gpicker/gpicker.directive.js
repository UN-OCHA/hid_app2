/*
 * gpicker, inspired from angular-google-picker
 */
(function () {
  'use strict';

  angular.module('app.gpicker', [])

  .provider('gpickerSettings', function () {
    this.apiKey   = null;
    this.clientId = null;
  ...
  .directive('gpicker', ['gpickerSettings', function (lkGoogleSettings) {
    return {
      restrict: 'A',
      scope: {
        onLoaded: '&',
      ...
      link: function (scope, element, attrs) {
        ...
        function onApiAuthLoad () {
          ...
          .then(function (authInstance) {
            if (scope.getToken) {
              return (scope.getToken)({authInstance: authInstance});
            }
          })
          .then(function (token) {
            accessToken = token;
